...
namespace HoloToolkit.Unity.InputModule.Utilities.Interactions
{
    ...
    /// </summary>
    public class TwoHandManipulatableObj : MonoBehaviour, IInputHandler, ISourceStateHandler
    {
        private LessonObjRef lessonObjRef = null;
        ...
        private void OnManipulationEnded()
        {
            ...
            Debug.Log("DONE MOVING STUFF");
            if(lessonObjRef != null)
            {
                updateLessonObjRef();
                Debug.Log("UPDATED: " + lessonObjRef);
            }
        }

        public void updateLessonObjRef()
        {
            lessonObjRef.SetTranslate(transform.position);
            lessonObjRef.SetRotation(transform.rotation);
            lessonObjRef.SetScaling(transform.localScale);
        }

        public void setLessonObjRef(LessonObjRef objRef)
        {
            this.lessonObjRef = objRef;
        }

        public LessonObjRef getLessonObjRef()
        {
            return this.lessonObjRef;
        }
    }
}
